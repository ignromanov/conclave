...
def _instance(tmp: Path) -> dict[str, Path]:
    ...
    p["ledger"] = _w(
        ...
        "entries:\n"
        "- duty_id: d_diff_preview_before_edit\n"
        "  session_id: 2026-08-06-neon\n"
        "  outcome: discharged\n"
        "  ts: '2026-08-06T12:00:00+00:00'\n",
    )
    p["projection"] = _w(
        tmp / "agent-memory" / "advisors" / OLD / "COMPUTED-DUTIES.md",
        f"# COMPUTED-DUTIES — {OLD}\n\n"
        f"> Generated by `engine duty project` (spec 091 §3). Do not hand-edit.\n",
    )
    # A SECOND advisor's ledger that merely NAMES the old id in a note. It must not
    # move: the decoy separates "carries the id in its path" from "mentions it".
    p["other_ledger"] = _w(
        tmp / "agent-memory" / "advisors" / OTHER / "duty-ledger.yaml",
        "entries:\n"
        "- duty_id: d_diff_preview_before_edit\n"
        "  session_id: 2026-08-06-audit\n"
        "  outcome: skipped\n"
        "  ts: '2026-08-06T12:00:00+00:00'\n"
        f"  note: deferred to {OLD}, who owns the model files\n",
    )
...
# ---------------------------------------------------------------------------
# conclave#99 — the per-advisor duty directory
# ---------------------------------------------------------------------------

def test_the_duty_ledger_arrives_under_the_new_id(applied):
    """The ledger's only statement of ownership is the directory it sits in.

    `duty-ledger.yaml` has no frontmatter and no `advisor:` key — the id is the
    parent directory's name and nothing else. Left unclassified, a rename forks the
    ledger: the new id starts an empty one on its next `/conclave:done` and the
    §5 health sweep reads half a history, which is how a duty attempted every
    session gets reported `dead`.
    """
    tmp, p, _ = applied
    assert not p["ledger"].exists(), "the ledger stayed under the retired id"
    moved = tmp / "agent-memory" / "advisors" / NEW / "duty-ledger.yaml"
    assert moved.is_file(), "the ledger did not arrive under the new id"
    assert "d_diff_preview_before_edit" in moved.read_text(), "entries lost in transit"
    assert not (tmp / "agent-memory" / "advisors" / OLD).exists(), (
        "the retired id's directory survived the move"
    )


def test_the_duty_projection_is_carried_not_deleted(applied):
    """REGEN would be the obvious class for a projection, and it is the wrong one.

    `COMPUTED-DUTIES.md` IS derived — but only by `engine duty project`, which no
    command, hook or lifecycle step invokes (`duty record` and `duty discharge` are
    the two `commands/done.md` calls). A REGEN drop therefore deletes an artifact
    that nothing rebuilds, which is a silent delete wearing a cache's clothes.
    """
    tmp, p, _ = applied
    assert not p["projection"].exists()
    moved = tmp / "agent-memory" / "advisors" / NEW / "COMPUTED-DUTIES.md"
    assert moved.is_file(), "the projection was dropped, and nothing regenerates it"


def test_the_carried_projection_is_reported_as_stale(applied):
    """Carried, and SAID to be stale — the projection still names the retired id.

    HISTORY does not rewrite prose, so the moved file's own title still reads
    `# COMPUTED-DUTIES — <old>`. That is correct for a record and wrong for a
    projection, so the gap is reported in the plan's `stale-after-rename` section
    with the command that closes it. A carried lie with no note is the failure this
    whole class of defect is made of.
    """
    tmp, _, r = applied
    moved = tmp / "agent-memory" / "advisors" / NEW / "COMPUTED-DUTIES.md"
    assert OLD in moved.read_text(), "prose was rewritten — this test measures the residue"
    assert "stale-after-rename" in r.stdout
    assert f"engine duty project --advisor {NEW}" in r.stdout, (
        "the plan carried a stale projection without naming the command that rebuilds it"
    )


def test_another_advisors_ledger_is_not_moved_by_a_mention(applied):
    """The decoy. `atlas-cro`'s ledger names the old id in a note and nothing else.

    Without this the rule above could be `any duty-ledger.yaml is HISTORY and moves`,
    which passes the two tests above while rewriting a second advisor's duty history
    — the exact shape of the 106 rename, where 29 of 30 cross-file rewrites were
    corruption and 1 was an identity.
    """
    tmp, p, _ = applied
