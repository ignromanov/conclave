...
def test_happy_path_flat(tmp_path):
    r = _create("--id", "vera-cto", "--role", "QA", "--color", "blue", tmp=tmp_path)
    assert r.returncode == 0, r.stderr

    agent_file = tmp_path / ".claude" / "agents" / "vera-cto.md"
    assert agent_file.exists(), "agent file not created"

    content = agent_file.read_text()
    assert "name: vera-cto" in content, "name must be bare id (flat layout)"
    assert "team." not in content, "no team.<id> references in flat agent-def"

    import json
    info = json.loads(r.stdout)
    assert info["id"] == "vera-cto"
    assert info["agent"].endswith("vera-cto.md")
...
def test_defaults(tmp_path):
    ...
    content = (tmp_path / ".claude" / "agents" / "vera-cto.md").read_text()
...
def test_collision(tmp_path):
    r1 = _create("--id", "vera-cto", "--role", "QA", "--color", "blue", tmp=tmp_path)
    assert r1.returncode == 0, r1.stderr

    r2 = _create("--id", "vera-cto", "--role", "QA", "--color", "blue", tmp=tmp_path)
    assert r2.returncode == 2
    assert "already exists" in r2.stderr


# 4. Invalid id: uppercase, slash, space → exit 1, stderr "invalid --id"
@pytest.mark.parametrize("bad_id", ["TestX", "has/slash", "has space", "testx", "vera-eng"])
...
def test_missing_role(tmp_path):
    r = _create("--id", "vera-cto", "--color", "blue", tmp=tmp_path)
    assert r.returncode == 1
...
def test_mint_provisions_personality(tmp_path):
    ...
    pmd = tmp_path / ".claude" / "skills" / "conclave-vera-cto" / "memory" / "personality.md"
    assert pmd.is_file(), "personality.md stub not provisioned on mint"
    body = pmd.read_text()
    assert "vera-cto" in body, "advisor id not substituted into personality stub"
...
def test_mint_writes_forge_block(tmp_path):
    ...
    wrapper = (tmp_path / ".claude" / "skills" / "conclave-vera-cto" / "SKILL.md").read_text()
