...
| Written by | Commands (`engine session close`, `engine file decision`, `engine briefing build`) | Wiki agent (`/knowledge:capture`) |
...
|-------------|-------------|------------|---------|
| Sessions | `agent-memory/advisors/sessions/YYYY-MM-DD-<id>-<slug>.md` | `engine session close` (via `/team.done`) | Work done, decisions, open items |
| Decisions | `agent-memory/advisors/decisions/YYYY-MM-DD-<slug>.md` | `engine file decision` | Cross-cutting Y-statement decisions |
| Mentions | `agent-memory/advisors/mentions/<recipient>/open/<id>.md` | `engine mention create` | Unresolved cross-advisor references |
| Mention archive | `agent-memory/advisors/mentions/<recipient>/archive/<id>.md` | `engine mention resolve` | Resolved mentions |
...
│   └── INDEX.md                   ← auto-regenerated by engine memory index
├── executors/
│   └── <executor-id>/MEMORY.md
├── hot.md                         ← ≤500 words, live cross-agent state
├── run-log/                       ← per-session execution logs (JSONL)
└── gh-cache/                      ← snapshot of GH issues/PRs (engine lifecycle gh-fetch)
```

The `gh-cache/` snapshot is written by `engine lifecycle gh-fetch` (a dedicated snapshot writer) so that
